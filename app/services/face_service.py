...
class MPService:
    _instance_mesh = None
    _instance_detector = None

    @classmethod
    def get_mesh(cls):
        if cls._instance_mesh is None:
            import mediapipe as mp
            cls._instance_mesh = mp.solutions.face_mesh.FaceMesh(
                static_image_mode=True,
                max_num_faces=5,
                refine_landmarks=True,
                min_detection_confidence=0.5,
            )
        return cls._instance_mesh

    @classmethod
    def get_detector(cls):
        if cls._instance_detector is None:
            import mediapipe as mp
            cls._instance_detector = mp.solutions.face_detection.FaceDetection(
                model_selection=0,
                min_detection_confidence=0.6,
            )
        return cls._instance_detector

# ── Canonical Face for Alignment ─────────────────────────
# Index mapping for stable points: [left_eye, right_eye, nose_tip, mouth_left, mouth_right]
_STABLE_POINTS = [33, 263, 1, 61, 291]


def _extract_encodings_from_frame(frame_bgr: np.ndarray) -> List[np.ndarray]:
    """
    Extract 1434-dim aligned face encodings for all faces in a BGR frame.
    Uses Procrustes-like alignment to canonicalize landmarks.
    """
    rgb = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)
    mesh = MPService.get_mesh()
    ...
    for face_landmarks in results.multi_face_landmarks:
        ...
        # 2. Canonical Alignment (Simple: Center on nose tip and scale by eye distance)
        # Nose tip is index 1
        nose_tip = pts[1]
        pts = pts - nose_tip
        
        # Eye indices: 33 (left), 263 (right)
        eye_dist = np.linalg.norm(pts[33] - pts[263])
        if eye_dist > 0:
            pts = pts / eye_dist
            
        # 3. Flatten and unit-normalize for cosine similarity
        encoding = pts.flatten()
...
def _extract_encoding_from_frame(frame_bgr: np.ndarray) -> Optional[np.ndarray]:
    """
    Backward-compatible method for single face enrollment.
    Returns the first extracted face encoding, or None.
    """
    encs = _extract_encodings_from_frame(frame_bgr)
...
class FaceService:
    ...
    def __init__(self, encodings_dir: str = 'face_encodings', tolerance: float = 0.12):
        ...
        # Temporal smoothing buffer: {track_id: {student_id: hit_count, last_seen: time}}
        self.recognition_buffer: Dict[int, Dict] = {}
        self.buffer_threshold = 3  # Hits required to confirm identity
        self.buffer_timeout = 2.0  # Seconds before clearing buffer
    ...
    def _match_encoding(self, query_enc: np.ndarray) -> RecognitionResult:
        ...
        Uses Weighted Top-K matching to reduce noise.
        """
        if not self.known_encodings:
            return RecognitionResult(is_unknown=True, message="No students enrolled")

        best_id = None
        min_dist = 1.0
        
        # Calculate distances for all known students
        scored_students = []
        for sid, enc_list in self.known_encodings.items():
            # Find closest sample for this student
            dists = [1.0 - float(np.dot(query_enc, st_enc)) for st_enc in enc_list]
            avg_dist = np.mean(sorted(dists)[:3]) # Top 3 average is more stable
        ...
        # stricter Unknown boundary to prevent false positives
        if best_id is not None and min_dist <= self.tolerance:
            return RecognitionResult(
                student_id=int(best_id),
                confidence=confidence,
                is_unknown=False,
                message=f"Match (d={min_dist:.3f})"
            )
        else:
            return RecognitionResult(
                is_unknown=True,
                confidence=confidence,
                message=f"Unknown (best={min_dist:.3f})"
            )

    def recognize_frame(self, frame_bgr: np.ndarray, use_buffer: bool = True) -> List[RecognitionResult]:
        """
        Recognize faces in a frame with optional temporal smoothing.
        """
        encodings = _extract_encodings_from_frame(frame_bgr)
        if not encodings:
            return [RecognitionResult(is_unknown=True, message="No face detected")]
            
        results = []
        for i, enc in enumerate(encodings):
            raw_res = self._match_encoding(enc)
            
            if not use_buffer or raw_res.is_unknown:
                results.append(raw_res)
                continue
                
            # Buffer Logic (Simple single-face tracking for now)
            track_id = i # In real production, we'd use a Centroid Tracker
            sid = str(raw_res.student_id)
            
            # Clean old buffer entries
            now = time.time()
            self.recognition_buffer = {tid: data for tid, data in self.recognition_buffer.items() 
                                       if now - data['last_seen'] < self.buffer_timeout}
            
            if track_id not in self.recognition_buffer:
                self.recognition_buffer[track_id] = {'sid': sid, 'hits': 1, 'last_seen': now}
                raw_res.is_unknown = True # Not confirmed yet
                raw_res.message = "Analyzing..."
            else:
                buf = self.recognition_buffer[track_id]
                if buf['sid'] == sid:
                    buf['hits'] += 1
                    buf['last_seen'] = now
                    if buf['hits'] < self.buffer_threshold:
                        raw_res.is_unknown = True
                        raw_res.message = f"Stabilizing... ({buf['hits']}/{self.buffer_threshold})"
                    else:
                        raw_res.message = "Confirmed ✓"
                else:
                    # Identity flip detected, reset hits
                    buf['sid'] = sid
                    buf['hits'] = 1
                    buf['last_seen'] = now
                    raw_res.is_unknown = True
                    raw_res.message = "Re-analyzing..."
            
            results.append(raw_res)
            
        return results

    def recognize_b64(self, image_b64: str, use_buffer: bool = True) -> List[RecognitionResult]:
        """Recognize faces from a base64 data-URL image."""
        frame = _b64_to_frame(image_b64)
        if frame is None:
            return [RecognitionResult(is_unknown=True, message="Decode failed")]
        return self.recognize_frame(frame, use_buffer=use_buffer)
